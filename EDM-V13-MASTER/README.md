...
**Pack version: V13.** Supersedes V12. Board sign-off 20 August 2026 —
...
| `DECISIONS.md` | Five settled positions on what may and may not be claimed. **Overrides anything you find in an older document.** |
...
## EDM OS status — read this before planning any development

The platform is a **well-built prototype, not a working product.** 182 API
endpoints exist; the front end calls none of them and renders from
`lib/data.ts`. There is no sign-in, no database migrations, no tests and no CI.
The architecture underneath is genuinely good — global auth guards, correct
multi-tenant isolation, boot-time config validation, a 51-model schema that
understands construction.

Full findings and the ten-step sequence to go-live:
`11-EDM-OS/EDM-OS-TECH-AUDIT.md`.

**Do not add screens.** Screens are the part that is finished.
